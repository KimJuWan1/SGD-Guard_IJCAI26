# siir_from_cache.py : semantic iterative identity refinement (train/infer)
...
# Losses (paper names + last iteration only)
...
def infer(args: argparse.Namespace):
    ...
    y_dummy = np.zeros((arc.shape[0],), dtype=np.int64)
    ds = EmbeddingCacheDataset(arc, face, y_dummy, force_l2norm=not args.no_input_l2)
...
def build_parser():
    ...
    t.add_argument("--arc_emb", type=str, required=True)
    t.add_argument("--face_emb", type=str, required=True)
    t.add_argument("--labels", type=str, required=True)
    t.add_argument("--arc_head", type=str, required=True)
    t.add_argument("--face_head", type=str, required=True)

    t.add_argument("--out_dir", type=str, default="./outputs_siir")
    ...
    t.add_argument("--no_input_l2", action="store_true")
    ...
    inf.add_argument("--arc_emb", type=str, required=True)
    inf.add_argument("--face_emb", type=str, required=True)
    inf.add_argument("--siir_ckpt", type=str, required=True)
    inf.add_argument("--out_dir", type=str, default="./gallery_out")
    inf.add_argument("--batch_size", type=int, default=128)
    inf.add_argument("--num_workers", type=int, default=0)
    inf.add_argument("--save_r12", action="store_true")
    inf.add_argument("--no_input_l2", action="store_true")
